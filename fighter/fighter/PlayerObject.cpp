...
{
	int PosX = GetCurX() - Map.GetDrawPosX();
	int PosY = GetCurY() - Map.GetDrawPosY();


	//------------------------------------------------------------
	// 그림자 표시.
	//------------------------------------------------------------
	g_cSprite.DrawSprite50(eSHADOW, PosX, PosY,bypDest,iDestWidth,iDestHeight,iDestPitch);
	...
	if ( IsPlayer() )
	{
		g_cSprite.DrawSpriteRed(GetSprite(), PosX, PosY,bypDest,iDestWidth,iDestHeight,iDestPitch);
	}
	else
	{
		g_cSprite.DrawSprite(GetSprite(), PosX, PosY,bypDest,iDestWidth,iDestHeight,iDestPitch);
	}

	//------------------------------------------------------------
	// HP 내용 표시.
	//------------------------------------------------------------
	g_cSprite.DrawSprite(eGUAGE_HP, PosX - 35, PosY + 9,bypDest,iDestWidth,iDestHeight,iDestPitch,GetHP());
...
void CPlayerObject::InputActionProc(void)
{
	...
	else
	{
		...
		{
		case dfACTION_ATTACK1:
			mpMoveStop (&clPacket, GetDirection (), GetCurX (), GetCurY ());
